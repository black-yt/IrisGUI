...
from scripts.memory import MEMORY_MANAGER_SYSTEM_PROMPT_TEMPLATE, HierarchicalMemory, build_memory_summary_user_prompt
...
class PromptTests(unittest.TestCase):
    def test_agent_system_prompt_covers_core_runtime_contracts(self):
        ...
        self.assertIn("`click`, `double_click`, `mouse_down`, `mouse_up`, `scroll`, `type`, `hotkey`, `wait`, `ask_input`, and `final_answer`", IRIS_SYSTEM_PROMPT)
        ...
        self.assertIn("Use `ask_input` only when you cannot safely continue", IRIS_SYSTEM_PROMPT)
        self.assertIn("Call `ask_input` by itself", IRIS_SYSTEM_PROMPT)
    ...
    def test_step_query_marks_mouse_grid_as_latest_local_id(self):
        ...
        self.assertIn("If using `click`, `double_click`, `mouse_down`, `mouse_up`, `scroll`, `type`, `hotkey`, `wait`, `ask_input`, or `final_answer`", query)
        self.assertIn("For `move`, include assistant content when moving toward a target", query)
        self.assertIn("call `ask_input` alone", query)
    ...
    def test_repair_prompt_keeps_key_action_content_requirement(self):
        ...
        self.assertIn("`ask_input`", TOOL_CALL_REQUIRED_RETRY_PROMPT)
    ...
    def test_memory_manager_prompt_preserves_user_input_exchanges(self):
        prompt = MEMORY_MANAGER_SYSTEM_PROMPT_TEMPLATE.format(initial_task="initial task")

        self.assertIn("user questions", prompt)
        self.assertIn("user-provided answers", prompt)
